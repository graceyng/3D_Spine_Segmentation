...
'''
dcmFileList = [pydicom.dcmread(filePath) for filePath in filePathList]
fileMap = {}
for fileIndex, dcmFile in enumerate(dcmFileList):
	seriesUID = dcmFile.SeriesInstanceUID
	if seriesUID in fileMap:
...
for index, seriesUID in enumerate(fileMap):
	outputFilePath = "D:\\test_transfer\\output%d.nii.gz" % index
	print(outputFilePath)
	dcmList = fileMap[seriesUID]

	dcmList = dicom2nifti.common.sort_dicoms(dcmList)
	dcmFile = dcmList[0]
	dcmFile.remove_private_tags()
	tagDict = {}
	# dcmFile.dir() gets tags
	for key in dcmFile.keys():
		if key in tagExcludeList:
			continue
		#tagDict[key] = getattr(dcmFile, key)
		tagDict[str(key)] = dcmFile.get(key).value
	affine = dicom2nifti.common.create_affine(dcmList)
	data = dicom2nifti.common.get_volume_pixeldata(dcmList)
	nii = nib.Nifti1Image(data, affine)
	nii.to_filename(outputFilePath)
	#print(affine)
	#results = dicom2nifti.convert_dicom.dicom_array_to_nifti(dcmList, None, reorient_nifti=True)
'''

#dicom2nifti.dicom_series_to_nifti(dcmDirPath, outputFilePath, reorient_nifti=True)
